...
function buildBodyNoLink({ userId, latitude, longitude, message, createdAt }) {
  ...
  lines.push(`Location: ${latitude}, ${longitude}`);
  lines.push('');
  lines.push(`Message: ${message}`);
  lines.push('');
  lines.push('Open the app or email for details.');
  return lines.join('\n');
}
...
(async function send() {
  try {
    ...
    const body = buildBodyNoLink(payload);
    console.log('Sending full formatted (no link) SMS to', TO);
    ...
    console.error('Failed to send full SMS (no link):', err.message || err);
    process.exit(1);
  }
})();
